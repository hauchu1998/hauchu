        ...
        <li className="relative border-l-2 border-gray-600 py-5">
            ...
            <div className="pl-10 ">
                <div className="flex justify-between items-center align-baseline">
                    {work.link?
                        <a href={work.link} className="text-lg font-semibold">{work.company}</a>
                        :
                        <p className="text-lg font-semibold">{work.company}</p>
                    }
                    <p className="text-sm italic">{work.location}</p>
                </div>
                <div className="flex justify-between text-sm italic text-gray-600">
                    <p>{work.title}</p>
                    <p>{work.duration}</p>
                </div>
                <div className={`${work.descriptions.length > 0 ? "block" : "hidden"} relative mt-4 text-sm text-gray-600 z-10`}>
                ...
                <div className={`${work.descriptions.length > 0 ? "block" : "hidden"} relative flex justify-end text-sm z-20`}>
                    <button className={`${show ? "hidden" : "block"} -mt-3 flex items-center gap-1 hover:underline`} onClick={() => setShow(true)}>
...
export default function Works() {
    return (
        <div id="work" className="w-full bg-neutral-300 flex flex-col justify-center items-center">
            <p className="mt-8 w-full font-bold text-3xl text-center underline underline-offset-2">WORK EXPERIENCE</p>
